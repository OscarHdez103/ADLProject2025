...
class CNN(nn.Module):
    def __init__(self, height: int, width: int, channels: int, class_count: int, max_epochs: int, min_dropout:float, max_dropout:float):
        super().__init__()
    ...
    def forward(self, image0: torch.Tensor, image1: torch.Tensor, epoch:int) -> torch.Tensor:
        ...
        d = self.min_dropout + (epoch/self.max_epochs) * (self.max_dropout - self.min_dropout)

        x = F.dropout(x, p=d, training=self.training)
        x = self.fc2(x)
        return x
