...
def main(args):
    ...
    if args.test:
        ...
        trainer_SPN.save_qualitative_examples()
    else:
...
class Trainer_SPN:
    ...
    def save_qualitative_examples(self, num_examples=3):
        output_dir = Path(self.summary_writer.log_dir) / "qualitative"
        output_dir.mkdir(parents=True, exist_ok=True)

        target_layer = self.model.conv31
        self.model.eval()
        ...
        for batch0, batch1, labels in self.val_loader:
            ...
            for i in range(len(labels)):
                ...
                heat0 = self.generate_gradcam(img0, img1, target_layer)
                heat1 = self.generate_gradcam(img1, img0, target_layer)

                save_image(img0, output_dir / f"{count}_anchor_label={label}_pred={pred}.png")
                save_image(img1, output_dir / f"{count}_comparator_label={label}_pred={pred}.png")

                cv2.imwrite(str(output_dir / f"{count}_anchor_gradcam.png"),
                            cv2.cvtColor(heat0, cv2.COLOR_RGB2BGR))
                cv2.imwrite(str(output_dir / f"{count}_comparator_gradcam.png"),
                            cv2.cvtColor(heat1, cv2.COLOR_RGB2BGR))

                count += 1
    class SiameseWrapper(nn.Module):
        def __init__(self, model, other_image):
            super().__init__()
            self.model = model
            self.other_image = other_image

        def forward(self, x):
            return self.model(x, self.other_image)

    def generate_gradcam(self, img, img1, target_layer):
        wrapped_model = self.SiameseWrapper(self.model, img1)
        c = GradCAM(model=wrapped_model, target_layers=[target_layer]) 
        new_img = c(input_tensor=img)[0]
        img_b = img[0].permute(1, 2, 0)
        img_b = img_b.cpu().numpy()
        img_b = np.float32(img_b)
        return show_cam_on_image(img_b, new_img, use_rgb=True)
